...
//        return IntStream.range(2, n).filter(i -> i % 2 == 0).boxed().collect(Collectors.toList());
        ...
        Collection<Integer> fillList = IntStream.range(2, n).filter(i -> i % 2 == 0).boxed().collect(Collectors.toList());
        return fillList;
...
//        return IntStream.iterate(n, i -> i >= 1, i -> i - 1).filter(i -> i % 2 != 0).boxed().collect(Collectors.toList());
        ...
        /* так же как и у метода fillEven выше */
        Collection<Integer> fillList = IntStream.iterate(n, i -> i >= 1, i -> i - 1).filter(i -> i % 2 != 0).boxed().collect(Collectors.toList());
        return fillList;
